...
const EProduct = () => {
  ...
  const UploadFile = async (e) => {
    ...
    try {
      const { data } = await axios.post("http://ec2-65-1-248-95.ap-south-1.compute.amazonaws.com:8886/api/product/add" , fd )
      console.log(data)
      fetc
    }catch(e) { 
      console.log(e)
    ...
    <>
      ...
      >
        ...
        <div className="d-flex gap-1">
          <input type="file" id="excel" style={{ display: "none" }} onChange={(e) => setFileUrl(e.target.files[0])} />
          <button
            onClick={() => {
              uploadExcel();
              UploadFile()
